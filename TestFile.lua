...
local options = {
    ...
    ignore_player_humanoids = false, -- will not lock onto/esp players (only if loop_all_humanoids is enabled)
    ...
    ignore_people = {
        ["LateApology"] = true, -- example of how you would exclude someone
        ["HannyChannnn"] = true
    },
    ...
    mouse_key = Enum.UserInputType["MouseButton2"], --Enum.UserInputType["MouseButton2"],
    ...
    looking_at_tracer = false, -- will show where a player is looking

    --https://www.google.com/search?q=color+picker
    lat_color = color3_rgb(255, 0, 0), -- color in rgb
    lat_thickness = 0.5,
...
local function get_players()
    ...
    if place == 292439477 then -- phantom forces
        ...
        if options.team_check then
            ...
            -- NEW BETTER METHOD ........
            if tostring(local_player.TeamColor) == "Bright blue" then
                return get_children(workspace.Players["Bright orange"])
            else
                return get_children(workspace.Players["Bright blue"])
            end
        else
...
local ignored_instances = {}--{workspace.EffectsJunk, workspace.BloodPools}

if workspace:FindFirstChild("Ignored", true) then
    table.insert(ignored_instances, workspace:FindFirstChild("Ignored", true))
end
...
getgenv().input_began = uis.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Keyboard then
        ...
        if input.KeyCode == Enum.KeyCode.KeypadMultiply then
            options.fov_circle = not options.fov_circle
        end
    end
            ...
            if char then
                ...
                if chosen then
                    ...
                    if start_aim then
                        ...
                        -- local mDir = chosen.part.Parent.Humanoid.MoveDirection
                        -- local Dist = (chosen.part.Position - local_player.Character.HumanoidRootPart.Position).Magnitude
                        -- local screen_mDir = to_screen(chosen.part.Position + mDir*(Dist * 0.015))
                        
                        if chosen.visible then
                            if options.mouse_emulation then
                                local MoveX = ((chosen.screen.X * 1 - mouse.X) / smoothness)
                                --  local MoveX = ((screen_mDir.X - mouse.X) / smoothness)
                                local MoveY = (chosen.screen.Y - (mouse.Y + 36)) / smoothness --+ Vector3.new(mDir.X, 0, mDir.Z)
                        ...
                        elseif options.rage_mode then
                            if options.mouse_emulation then
                                local MoveX = screen_size.X / 4
                                local MoveY = (chosen.screen.Y - (mouse.Y + 36)) * screen_height(mouse.Y)
                                mousemoverel(MoveX, MoveY)
                            elseif options.camera_cframe then
                                cam.CFrame = cframe_new(cam.CFrame.p, chosen.part.Position)
                            end
